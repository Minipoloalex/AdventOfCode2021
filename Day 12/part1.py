...
def possible_paths(input_list):
    ...
    number_paths = calculate_path_number(input_list, from_start, {}, "start")
...
def calculate_path_number(input_list, from_before, count, current):     # current variable is only for debugging
    n = 0
    ...
    for next_cave in from_before:
        ...
        if not(is_small_cave(next_cave)) or times_visited == 1:
            n += calculate_path_number( input_list, from_paths(input_list, next_cave), new_count, next_cave)

        new_count[next_cave] -= 1       # small cave visited more than once
